<template>
  <div calss="terminal-info">
    ...
    <section class="m-t-20 info labelAll-content">
      ...
      <template v-if="activePlaylist.tagged.length > 0">
        <ul class="labelAll-box">
          <li class="label-options" v-for="(item,index) in activePlaylist.tagged" :key="index">{{ item.tag_name }}</li>
        </ul>
      </template>
      <template v-else>
        <div>未选择标签</div>
...
export default {
  ...
  computed: {
    checkedDefault () {
      let treesGroup = []
      for (let inx in this.activePlaylist.trees) {
        treesGroup.push(this.activePlaylist.trees[inx].id)
      }
      return treesGroup
    }
  },
