<template>
  <div class="clearfix" id="templateContent">
    ...
    <div v-if="create" class="m-b-15">
      ...
      <el-input style="display:inline-block;width:200px;" placeholder="请输入名称过滤" prefix-icon="el-icon-search" v-model="searchQuery" maxlength="20" show-word-limit>
      </el-input>
    </div>
    ...
    <div v-for="(item, index) in list" :key="item.id" class="item">
      ...
      <div class="item-bottom">
        ...
        <div style="padding-bottom:16px;margin-top:-8px;">
          <router-link title="编辑" v-if="item.status !== '1'" :to="{ name: 'sceneedit', params: {id: item.id, scene: item} }">
