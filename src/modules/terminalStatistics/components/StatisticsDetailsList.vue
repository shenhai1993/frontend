<template>
  <div class="role-warpper">
    ...
    <div class="rateContent">
      <el-table :data="tableData" style="width: 100%" highlight-current-row :height="tableHeight" v-loading="detailsloadingStatus">
        <el-table-column prop="element_name" label="素材名称"></el-table-column>
        <el-table-column prop="playlist_name" label="播出单名称"></el-table-column>
        <el-table-column prop="content_name" label="内容"></el-table-column>
        <el-table-column prop="terminal_number" label="终端编号"></el-table-column>
        <el-table-column prop="seconds" label="时长"></el-table-column>
        <el-table-column prop="played_day" label="播放时间"></el-table-column>
      </el-table>
    </div>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        detailsList: state => state.terminalStatistics.detailsList,
        detailsloadingStatus: state => state.terminalStatistics.detailsloadingStatus
      })
    },
    watch: {
      setTerminals(val) {
    ...
    methods: {
      ...mapMutations(['SET_DETAILSLOADING_STATUS']),

      ...mapActions(['getDetailsList']),
      getStatisticsElement(id, start , end){
        this.SET_DETAILSLOADING_STATUS(true)
        this.getDetailsList({ type:id, starttime:start, endtime:end, page:this.currentPage, perPage: this.customizePages}).then(res=>{
