<template>
  <el-drawer
    ...
    v-loading="loading">
    <el-container>
      <el-header>{{currentModelData.special_name}}</el-header>
      <el-container>
        <el-aside width="50%">
          <div class="card" style="margin-top: 15px;" :style="{ width: simsResetWH[0], height: simsResetWH[1] }">
            <component
              :is="componentName"
              :comData="currentModelData"
              @currentModelDataEmit="currentModelDataEmit">
            </component>
          </div>
        </el-aside>
        <el-main v-if="currentModelData.special_type == 'product-model'">
          <steps-list
            :comData="currentModelData"
            @stepsEmit="stepsEmit"
        ...
        <el-main v-else-if="currentModelData.special_type == 'ad-carousel'">
          <ad-update
            :adList="adListData"
            @adUpdateEmit="adUpdateEmit">
          </ad-update>
        </el-main>
        <el-main v-else-if="currentModelData.special_type == 'logo-model'">
          <logo-update
...
export default {
  ...
  methods: {
    ...
    getCurrentModelData () {
      this.currentModelData = JSON.parse(JSON.stringify(this.currentData))
      this.componentName = 'edit-' + this.currentModelData.special_type
      this.top = this.currentModelData.top
      if (this.id > -1) {
        this.currentModelData.top = 0
        this.productId.id = this.currentModelData.comData[0].id
      }
      if (this.currentModelData.special_type == 'ad-carousel') {
        this.adListData = this.currentModelData.comData
      }
      if (this.currentModelData.special_type == 'logo-model') {
        this.logoListData = this.currentModelData.comData
      }
    },

    currentModelDataEmit (data) {
      this.currentModelData = JSON.parse(JSON.stringify(data))
    },

    getCommitModelData () {
      let data = JSON.parse(JSON.stringify(this.currentModelData))
    ...
    commitModelData () {
      if (this.productId.id == 0 && this.currentModelData.special_type == 'product-model') {
        this.$message({
      ...
      if (this.id == -1) {
        this.commitTempData(0)
      } else {
        this.commitTempData(this.currentModelData.id)
      }
    },
    ...
    commitTempData (id) {
      ...
      if (id == 0) {
        this.top = Terminaltemp.getSimsViewModelTop(this.simsViewData, this.simsViewData.length)
      }
    ...
    stepsEmit (val) {
      ...
      this.currentModelData.comData = []
      this.$nextTick(() => {
        this.currentModelData = Object.assign({}, this.currentModelData, {comData: comData})
      })
    },
    ...
    adUpdateEmit (data) {
      ...
      this.currentModelData.comData = dataArr
    },
