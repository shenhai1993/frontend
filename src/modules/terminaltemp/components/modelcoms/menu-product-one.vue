<template>
  <div
    ref="menuProductOne"
    class="simscoms menu-product-one"
    ...
    <div class="one-aside">
      <ul>
        <li
          ...
          :class="{'one-aside-active': index == 0}">
          {{value}}
        </li>
      </ul>
    </div>
    <div class="one-main">
      <div
        class="one-main-list one-main-list-add"
        @click="addProduct"
        :style="{
...
export default {
  ...
  mounted () {
    this.productHeight = document.getElementsByClassName('one-main-list')[0].offsetWidth * 0.44
  },
  ...
  methods: {
    ...
    cencelEditFunc (event) {
      ...
      if (this.$refs.menuProductOne && !this.$refs.menuProductOne.contains(e.target)) {
        const path = e.path
...
<style lang="scss" scoped>
  .menu-product-one {
    ...
    .one-main::-webkit-scrollbar {
      width: 0px;
      height: 0px;
    }
    .one-main::-webkit-scrollbar-thumb {
      background: transparent;
    }
    .one-main::-webkit-scrollbar-track {
      background: transparent;
    }
  }
  .level-state, .one-main-list:hover {
