...
let MultiCompResize = function (container) {
  ...
  mResizer.resizeMove = function (element, b, e) {
    ...
    switch (b) {
      case c.RESIZE_E: // 右
        s.width = (e.center.x + window.scrollX - 229) / this.zoom - point.left
        this.CheckESES(s)
        m.width = s.width
        break
      case c.RESIZE_S: // 下
        s.height = (e.center.y + window.scrollY - 71) / this.zoom - point.top
        this.CheckESES(s)
        m.height = s.height
        break
      case c.RESIZE_SE: // 右下
        s = {
          width: (e.center.x + window.scrollX - 229) / this.zoom - point.left,
          height: (e.center.y + window.scrollY - 71) / this.zoom - point.top
        }
        this.CheckESES(s)
        m.width = s.width
        m.height = s.height
        break
      case c.RESIZE_W: // 左
        s.l = (e.center.x + window.scrollX - 229) / this.zoom
        s.w = point.right - s.l
        this.CheckW(s)
        m.width = s.w
        m.left = s.l
        break
      case c.RESIZE_N: // 上
        s.t = (e.center.y + window.scrollY - 71) / this.zoom
