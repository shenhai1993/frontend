    ...
    :style="{ width: selectItem.width + 'px', height: selectItem.height + 'px', zoom: zoom}">
      ...
      <div v-for='data in selectItem.data' :key="data.id">
        <region :key="data.id"
          ...
          <!-- {{ data.type }} -->
          <i v-if="data.type === icon.type && canEdit" v-show="!islistsPage"
              v-for="(icon, index) in elementsIcon" class="i_type"
              :class="icon.icon" :key="index"
              :style="{zoom: (1 / zoomNum) * 0.4}"
              @click="onAddClicked">
            <i class="fa fa-plus-circle"></i>
          </i>
          <div class='materialContent' v-if="data.type === 'video' && (selectedFiles[data.id] && selectedFiles[data.id].length)">
            <video class='materialPlaylist' object-fit="fill" :src="checkOverElement[data.id]" >
              <!-- <source :src="selectedFiles[data.id][0].path" type="video/mp4"> -->
              <!-- <source :src="checkOverElement[data.id]" type="video/mp4">
              您的浏览器不支持 video 元素。 -->
            </video>
          </div>
          <div class='materialContent' style="width:100%;height: 100%;" v-if="data.type === 'picture' && (selectedFiles[data.id] && selectedFiles[data.id].length)">
...
export default {
  ...
  data () {
    return {
      ...
      elementsIcon: [
        ...
        {icon: 'el-icon-tickets', type: 'webpage'}
      ],
      ismpdpPage: false,
      islistsPage: false,
      zoomNum: 1
  ...
  computed: {
    ...mapState({
      ...
      checkOverElement: state => state.mpdp.checkOverElement
    })
  },
  mounted () {
    let me = this
    ...
    me.islistsPage = this.$route.path.indexOf('/mpdp/playlist/lists') > -1
  },
  methods: {
    ...mapActions([
      'storeAutoZoom'
