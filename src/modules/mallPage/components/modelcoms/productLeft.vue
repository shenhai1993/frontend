<template>
  <div
    ...
    <div class="product-left-img"><el-image v-if="productItem.img_url" :src="productItem.img_url" alt="图片" :preview-src-list="[productItem.img_url]"></el-image><div v-if="productItem.stock" class="product-left-stock" :style="{ fontSize: (currCustomData.size - 2) / 10 + 'rem' }">剩余：{{productItem.stock}}</div></div>
    <div class="product-left-text">
      <div class="product-left-name">{{productItem.name}}</div>
      <div v-if="productItem.spec" class="product-left-name" :style="{ fontSize: (currCustomData.size - 2) / 10 + 'rem' }">{{'（' + productItem.spec + '）'}}</div>
